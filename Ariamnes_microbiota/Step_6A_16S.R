########################################################################################################
########################################################################################################
#### Step 6 A: Make the OTU Table for the samples #### 
...
metadata <- read.table(paste(path,"metadata/metadata_NC.csv",sep=''), header=T, sep=";")

setwd(paste(path,"OTU_table/",sep=''))

for(k in indexes){
  print(k)
  table <- read.table(file=paste(path,"results_wNC/OTU","_16s_taxonomy.txt",sep=""), header=F, sep="\t")
  ...
  write.table(table, file=paste(path,"OTU_table/OTU_S",k,"_16S_abundance.csv",sep=""),row.names=F, sep=";")
  rm(table)
}

write.table(metadata, file=paste(path,"metadata/metadata.csv",sep=''),row.names=F,sep=";")