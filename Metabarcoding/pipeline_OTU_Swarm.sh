...
#####################################################################################################
######################  PERFORM OTU CLUSTERING FROM  METABARCODING DATASETS USING SWARM #############
#####################################################################################################


## You first need to install PYTHON3, VSEARCH, CUTADAPT and SWARM (v3)
...
# see example in https://github.com/BPerezLamarque/Scripts/blob/master/Metabarcoding/mapping_example_ITS.txt
...
##  Step 3-C: SWARM clustering

swarm=/PATH_TOWARD_SWARM/swarm-3.0.0-macos-x86_64/bin/swarm


THREADS=2  #number of cores

$swarm \
    -d 1 -f -t ${THREADS} --bloom-bits 4 -z \
    -i "swarm_"$Marker"_otu_1f.struct" \
    -s "stats_"$Marker"_swarm_1f.txt" \
    -w "reads_"$Marker"_swarm_1f.fa" \
    -o "reads_"$Marker"_mapped_swarm_1f.txt" < "reads_"$Marker"_derep.fa"


# Sort representatives
vsearch --fasta_width 0 \
    --sortbysize "reads_"$Marker"_swarm_1f.fa" \
    --output "reads_"$Marker"_swarm_1f_final.fa"
...
vsearch --usearch_global "reads_"$Marker"_swarm_1f_nonchimeras.fa" \
--threads 2 \
--dbmask none \
--qmask none \
--rowlen 0 \
...
--id 0.5 \
--iddef 4 \
--userout "taxonomy_otu_"$Marker"_swarm_1f.txt"
...
REPRESENTATIVES="reads_"$Marker"_swarm_1f_final.fa"
UCHIME="reads_"$Marker"_swarm_1f.uchime"
ASSIGNMENTS="taxonomy_otu_"$Marker"_swarm_1f.txt"
OTU_TABLE="OTU_table_"$Marker"_swarm_1f.txt"
...
TABLE="OTU_table_"$Marker"_swarm_1f.txt"
