...
#####################################################################################################
######################  PERFORM ASV CLUSTERING FROM  METABARCODING DATASETS                ##########
#####################################################################################################
...
##  Step 3-C: Denoising into amplicons single variants (ASV)
...
## Denoising
vsearch --cluster_unoise "reads_"$Marker"_sorted.fa" --minsize 8 --centroids "reads_"$Marker"_ASV.fa" --uc "clusters_"$Marker"_ASV.uc"  --sizein --sizeout



# Make file that mapped reads to OTUs
python3 $path_scripts/map2qiime.py "clusters_"$Marker"_ASV.uc" > "reads_"$Marker"_mapped_ASV.txt" # python script present in https://github.com/BPerezLamarque/Scripts/blob/master/Metabarcoding/map2qiime.py


# One line per OTU sequence
vsearch --fasta_width 0 \
--sortbysize "reads_"$Marker"_ASV.fa" \
--output "reads_"$Marker"_ASV_final.fa"


# Make stats file
python3 $path_scripts/make_stats.py "reads_"$Marker"_ASV_final.fa" > "stats_"$Marker"_ASV.txt" # python script present in https://github.com/BPerezLamarque/Scripts/blob/master/Metabarcoding/make_stats.py
...
vsearch --usearch_global "reads_"$Marker"_ASV_nonchimeras.fa" \
--threads 2 \
--dbmask none \
--qmask none \
--rowlen 0 \
...
--id 0.5 \
--iddef 4 \
--userout "taxonomy_"$Marker"_ASV.txt"
...
REPRESENTATIVES="reads_"$Marker"_ASV_final.fa"
UCHIME="reads_"$Marker"_ASV.uchime"
ASSIGNMENTS="taxonomy_"$Marker"_ASV.txt"
OTU_TABLE="OTU_table_"$Marker"_ASV.txt"
